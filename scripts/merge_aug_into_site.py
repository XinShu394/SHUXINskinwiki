...
def main():
    ...
    records_path = SITE / "_new_aug_records.json"
    meta_entries_path = SITE / "_new_aug_meta.json"
    ...
    keep_data = [x for x in old_data if x.get("weapon") != "AUG"]
    merged_data = keep_data + new_records

    for cover in covers:
        if cover.get("weapon") == "AUG":
            cover["enabled"] = True
    ...
    print(f"merged aug records: {len(new_records)}")

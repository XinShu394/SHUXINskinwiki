...
def main():
    ...
    records_path = SITE / "_new_qbz95_records.json"
    meta_entries_path = SITE / "_new_qbz95_meta.json"
    ...
    keep_data = [x for x in old_data if x.get("weapon") != "QBZ95"]
    merged_data = keep_data + new_records

    for cover in covers:
        if cover.get("weapon") == "QBZ95":
            cover["enabled"] = True
    ...
    print(f"merged qbz95 records: {len(new_records)}")

...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("Usage: python merge_template_weapon_into_site.py <weapon_name>")
    ...
    records_path = SITE / f"_new_{key}_records.json"
    meta_entries_path = SITE / f"_new_{key}_meta.json"
    ...
    keep_data = [x for x in old_data if x.get("weapon") != weapon]
    merged_data = keep_data + new_records

    for cover in covers:
        if cover.get("weapon") == weapon:
            cover["enabled"] = True
    ...
    print(f"merged records: {len(new_records)}")

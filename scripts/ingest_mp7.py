...
对 MP7/ 目录下每个皮肤文件夹：
- Snipaste_*.png        -> MP7-{id}_A.png  （不裁剪）
...
用法：
  python scripts/ingest_mp7.py              # 处理所有文件夹
  python scripts/ingest_mp7.py --folder UG橙蓝   # 只处理指定文件夹
  python scripts/ingest_mp7.py --dry-run         # 仅预览，不写文件
"""
...
def process_folder(folder: Path, dry_run: bool) -> bool:
    ...
    skin_id = f"MP7-{normalized_code}-{serial}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MP7 皮肤图片标准化")
    parser.add_argument("--folder", help="只处理指定文件夹名（如 UG橙蓝）")
    parser.add_argument("--dry-run", action="store_true", help="仅预览，不写文件")
    args = parser.parse_args()

    if not MP7_DIR.exists():
        sys.exit(f"目录不存在: {MP7_DIR}")

    if args.folder:
        targets = [MP7_DIR / args.folder]
        missing = [t for t in targets if not t.is_dir()]
        if missing:
            sys.exit(f"文件夹不存在: {missing[0]}")
    else:
        targets = sorted([p for p in MP7_DIR.iterdir() if p.is_dir() and p.name not in IGNORED])
